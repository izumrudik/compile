...
def run_command(command:list[str],config:Config):
	if not config.silent:
		print(f"""[CMD] {' '.join(command)}""")
	return subprocess.call(command)

def run_assembler(config:Config):
	run = lambda x:run_command(x,config)
	ret_code = run(['nasm',config.output_file+'.asm','-felf64'])
	if ret_code != 0:
		print(f"ERROR: nasm exited abnormaly with exit code {ret_code}")
		exit(14)

	ret_code = run(['ld','-o',config.output_file+'.out',config.output_file+'.o'])
	if ret_code != 0:
		print(f"ERROR: GNU linker exited abnormaly with exit code {ret_code}")
		exit(15)
	
	ret_code = run(['chmod','+x',config.output_file+'.out'])
	if ret_code != 0:
		print(f"ERROR: chmod exited abnormaly with exit code {ret_code}")
		exit(16)
...
def main():
	...
	if config.run_file:
		exit(run_command([config.output_file+'.out'],config))
